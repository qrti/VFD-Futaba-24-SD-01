...
.equ    CMD_SUPON   =   137                     ;        supply on
.equ    CMD_SUPOFF  =   138                     ;               off
.equ    CMD_SETCUCH =   139                     ;        set custom char
.equ    CMD_SCLON   =   140                     ;        scroll on
.equ    CMD_SCLOFF  =   141                     ;               off
...
procData:
        ...
        sbis    GPIOR0,SCROLL               ;scroll active?
        rjmp    pd01                        ;no, jump

        mov     ZL,XL                       ;copy write pointer
        ldi     a5,1                        ;scroll timer start value
        std     Z+(NUMDIGITS*2),a5          ;             store 

        subi    XL,(-NUMDIGITS)             ;scroll pointer

pd01:   st      X+,a4                       ;store char

        sbic    GPIOR0,SCROLL               ;scroll active?
        subi    XL,NUMDIGITS                ;yes, restore pointer

        cpi     XL,(text+NUMDIGITS)         ;text buffer roll over
        brlo    PC+2
        ldi     XL,text
...
ascTab:
.DB $00,$00,$00,$00,$00,$00,$00,$7a,$00,$00,$00,$60,$00,$60,$00,$14
